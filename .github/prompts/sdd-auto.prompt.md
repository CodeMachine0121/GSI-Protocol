...
> **💡 注意：** 自動模式不包含選用的 integration tests。如需測試先行開發，請使用手動工作流程並加入 `@workspace /sdd-integration-test`。
...
- **Integration Tests**：如需測試先行開發，請使用手動工作流程
  - `@workspace /sdd-spec` → `@workspace /sdd-arch` → `@workspace /sdd-integration-test` → `@workspace /sdd-impl` → `@workspace /sdd-verify`
