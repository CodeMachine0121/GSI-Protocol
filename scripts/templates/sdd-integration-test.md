...
1. **規格階段**：定義 feature file（`__CMD_PREFIX__sdd-spec`）
2. **架構階段**：設計技術架構（`__CMD_PREFIX__sdd-arch`）
3. **測試階段**（本指令）：生成 integration tests（🔴 紅燈）
4. **實作階段**：實作功能（`__CMD_PREFIX__sdd-impl`）
5. **驗證階段**：確認測試通過（`__CMD_PREFIX__sdd-verify` → 🟢 綠燈）
