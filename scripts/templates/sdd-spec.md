...
**`.gsi/vip-discount/PRD.md`：**

```markdown
# PRD: VIP 折扣系統

## 功能描述
給予 VIP 客戶折扣優惠，以提高客戶忠誠度和重複購買率。

## 業務規則
- VIP 使用者購買金額超過 $100 享 20% 折扣
- 非 VIP 使用者無折扣
- 購買金額須為正數
...
- **Given** 使用者是 VIP
- **When** 使用者購買 1000 美元
- **Then** 最終價格應該是 800 美元，折扣 200 美元

### S-2: 非 VIP 使用者無折扣
- **Given** 使用者是 NORMAL
- **When** 使用者購買 1000 美元
- **Then** 最終價格應該是 1000 美元，折扣 0 美元

### S-3: 購買金額低於門檻無折扣
- **Given** 使用者是 VIP
- **When** 使用者購買 50 美元
- **Then** 最終價格應該是 50 美元，折扣 0 美元

### S-4: 處理無效的購買金額
- **Given** 使用者是 VIP
- **When** 使用者購買 -100 美元
- **Then** 系統應該拒絕並回應 400 錯誤
```

**`.gsi/vip-discount/vip-discount.feature`：**
...
Feature: VIP 折扣系統
  ...
  Scenario: 對 VIP 使用者套用折扣
    ...
    And the request body is:
      """
      { "userId": 1, "userType": "VIP", "amount": 1000 }
      """
    Then the response status should be 200
    Then the response body should contain field "finalPrice" with value "800"
    Then the response body should contain field "discount" with value "200"

  Scenario: 非 VIP 使用者無折扣
    When I send a "POST" request to "/api/orders"
    And the request body is:
      """
      { "userId": 2, "userType": "NORMAL", "amount": 1000 }
      """
    Then the response status should be 200
    Then the response body should contain field "finalPrice" with value "1000"

  Scenario: 購買金額低於門檻無折扣
    When I send a "POST" request to "/api/orders"
    And the request body is:
      """
      { "userId": 1, "userType": "VIP", "amount": 50 }
      """
    Then the response status should be 200
    Then the response body should contain field "finalPrice" with value "50"

  Scenario: 處理無效的購買金額
    When I send a "POST" request to "/api/orders"
    And the request body is:
      """
      { "userId": 1, "userType": "VIP", "amount": -100 }
      """
